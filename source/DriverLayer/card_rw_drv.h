...
typedef struct CPU_SENDER_CARD_S
{
	...
	uint8_t admin_key_type;      //默认密钥类型 0无密钥，1默认，2第三方(默认密钥需删除所有文件，第三方密钥则只需创建)
	uint8_t admin_ex_key_cnt;    //厂家外部认证密钥号 0
	uint8_t admin_ex_key[16];    //厂家默认密钥 16个0XFF
	uint8_t adf[2];              //adf文件夹标识
...
/* IC卡发卡通信数据结构*/
typedef struct IC_SENDER_CARD_S
{
	uint8_t cmd_aux;             //辅助指 AUTO_SEND( 默认参数初始化)，PARAM_SEND(传入参数初始化)
	uint8_t uid[4];              //卡号
	uint8_t block;               //绝对块号0~63
	uint8_t admin_key_type;      //厂家默认密钥类型
	uint8_t admin_key[6];        //厂家默认密钥
	uint8_t key_A[6];            //A密钥
	uint8_t authority[4];        //权限值
	uint8_t key_B[6];            //B密钥
	uint8_t result;
}IC_SENDER_CARD_T;
