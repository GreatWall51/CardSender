...
*
* Desc:      发CPU卡默认参数初始化（必须传入uid,admin_key_type）
* Param(in):
* Param(out):
* Return:
* Global:
...
//	  scan_ISO14443A_card ( cpu_para->uid );由检测到卡事件时传入
	...
	if(cpu_para->admin_key_type == 1)//默认类型
	{
		cpu_para->admin_ex_key_cnt = 0;
		memset(cpu_para->admin_ex_key,0xff,16);
	}
...
#if DEBUG_IC_SENDER
static void print_ic_default(IC_SENDER_CARD_T *card_info)
{
	if(!card_info) return ;
	debug("\r\n");
	debug("aux_cmd        : %x\r\n",card_info->cmd_aux);
	debug("uid            : ");debug_hex(card_info->uid,4);
	debug("block          : %x\r\n",card_info->block);
	debug("admin_key_type : %x\r\n",card_info->admin_key_type);
	debug("admin_key      : ");debug_hex(card_info->admin_key,6);
	debug("key_a          : ");debug_hex(card_info->key_A,6);
	debug("authority      : ");debug_hex(card_info->authority,4);
	debug("key_b          : ");debug_hex(card_info->key_B,6);
}
#endif
void ic_card_default_init(IC_SENDER_CARD_T *card_info)
{
	uint8_t *anticopy;
	uint8_t mastery[4]={0x78,0x77,0x88,0x69};//存取控制位默认值
	uint8_t admin_key[6]={0xff,0xff,0xff,0xff,0xff,0xff};
	if(!card_info) return ;
	
	config_get_data(CONFIG_ANTICOPY,&anticopy);
	if(anticopy[0] >= 0x80 && anticopy[0] <= 0x8f)
	{
		card_info->block = (((anticopy[0]-0x80)<<2)+3);
	}
//	else if(anticopy[0] == 0xa0)
//	{
//		
//	}
	card_info->admin_key_type = ISO14443A_CMD_AUTH_A;
	memcpy(card_info->admin_key,admin_key,6);
	memcpy(card_info->authority,mastery,4);
	gen_key_ic(0,card_info->uid, (card_info->block >> 2),card_rw_config.prj_num,card_info->key_A);//计算卡的扇区密钥keyA
	gen_key_ic(1,card_info->uid, (card_info->block >> 2),card_rw_config.prj_num,card_info->key_B);//计算卡的扇区密钥keyB
	
#if DEBUG_IC_SENDER
	print_ic_default(card_info);
#endif
}
...
*******************************************************************************/
CARD_ERR0R_ENUM ic_card_sender ( IC_SENDER_CARD_T *card_info )
{
	uint8_t step;
	
	if(!card_info) return NONE;
	
	for(step = 0;step<IC_SENDER_STEP_MAX;step++)
	{
		card_info->result = NONE;
		switch(step)
        {
			case 0:
				card_info->result = card_ic_select(card_info->uid);
			    if(card_info->result)
				{
					card_info->result = CARD_ID_ERR;
					step = IC_SENDER_STEP_MAX;
				}
			break;
			
			case 1 :/*用厂家默认密钥对相应扇区进行认证,若认证通过则该卡未发过*/
			{
				if(card_info->cmd_aux == AUTO_SEND)//自动初始化，初始化默认参数
				{
					ic_card_default_init(card_info);
				}
				card_info->result = ISO14443A_Authen(card_info->uid,card_info->block,card_info->admin_key,card_info->admin_key_type);
				if(card_info->result)//厂家默认密钥认证失败,用立林默认密钥认证
				{
					//密钥认证失败后，必须要重新寻卡
					card_info->result = card_ic_select(card_info->uid);
					if(card_info->result)
					{
						...
						step = IC_SENDER_STEP_MAX;
						#if DEBUG_IC_SENDER
						debug("select card error\r\n");
						#endif
						continue;
					}
					card_info->result = ISO14443A_Authen(card_info->uid,card_info->block,card_info->key_B,ISO14443A_CMD_AUTH_B);
					if(card_info->result == ISO14443A_SUCCESS)//已经发过卡
					{
						card_info->result = CARD_NONE_ERR;
					    step = IC_SENDER_STEP_MAX;
						#if DEBUG_IC_SENDER
						debug("use default key authen fail,but use leelen keyb authen success!\r\n");
						#endif
					}
					else
					{
						card_info->result = IC_AUTH_ERR;
					    step = IC_SENDER_STEP_MAX;
						#if DEBUG_IC_SENDER
						debug("use default key and leelen keyb authen fail!\r\n");
						#endif
					}
				}
				else
				{
					#if DEBUG_IC_SENDER
					debug("use default key authen sueecss!\r\n");
					#endif
				}
			}
			break;
			...
			case 2 :/*对相应扇区进行写密钥*/
			{
				...
				memcpy(write_data,card_info->key_A,6);
				memcpy(&write_data[6],card_info->authority,4);
				memcpy(&write_data[10],card_info->key_B,6);
				
				#if DEBUG_IC_SENDER
				debug("write data:(block = %d)\r\n",card_info->block);
				debug_hex(write_data,16);
				#endif
				
				card_info->result = ISO14443A_Write(card_info->block,write_data);
				if(card_info->result)
				{
					card_info->result = IC_WRITE_ERR;
					step = IC_SENDER_STEP_MAX;
					#if DEBUG_IC_SENDER
					debug("write block = %d error!\r\n",card_info->block);
					#endif
				}
				else
				{
					#if DEBUG_IC_SENDER
					debug("write block = %d success!\r\n",card_info->block);
					#endif
	...
	return (CARD_ERR0R_ENUM)card_info->result;
}
/******************************************************************************
...
uint8_t cpu_card_sender ( CPU_SENDER_CARD_T* card_info )
{
    ...
    for ( step = CHECK_HAVE_CARD; step <= ( STEP_MAX + 1 ); step++ )
    {
        ...
        case SELECT_MF://进入主目录
            ...
            {
                error_cnt = 0;
				if(card_sender->admin_key_type == 0)//没有密钥
				{
					step = DELETE_ALL-1;
				}
                #if DEBUG_CPU_SENDER
		...
		case DEFAULE_AUTH://用厂家默认密钥或第三方密钥进行认证
        {
            card_sender->result = ExAuthenticate ( card_sender->admin_ex_key_cnt, card_sender->admin_ex_key );
            if ( card_sender->result )
            {
            ...
            else
            {
                ...
                #endif
				if(card_sender->admin_key_type == 1)//默认密钥，则删除MF下所有文件再创建
				{
					step = DELETE_ALL-1;//跳过内部认证,直接删除所有文件
				}
				else if(card_sender->admin_key_type == 2)//第三方密钥，直接创建ADF
				{
...
void card_rw_drv_init ( void )
{
	...
	memcpy(config.prj_num,anticopy+1,16);
//	debug("project num:");
//	debug_hex(config.prj_num,16);
