...
1、读卡操作先进行初始化寄存参数，默认支持IC卡读卡号，void card_read_drv_init(void),移植需改

   如果要有防复制或者读取身份证功能需设置参数，void card_read_set_para(CARD_READER_CONFIG *card_para)。

2、在主函数里调用扫描函数，void card_read_drv_real_time(void)
...
/*****************************************************************************
 函 数 名  : card_set_para
 功能描述  : 卡处理的参数设置
 输入参数  : card_para,是否支持读身份证和IC卡防复制
 输出参数  : 无
 返 回 值  :
 调用函数  :
 被调函数  :
...
uint8_t chevt_have_card_evt(void)
{
	...
	if(old_sta == new_sta)//无事件
	{
		return CARD_EVT_NONE;
	}
	else if(old_sta > new_sta)//卡移离
	{
		old_sta = new_sta;
		return CARD_EVT_EXIT;
	}
	else if(old_sta < new_sta)//检测到卡
	{
		old_sta = new_sta;
